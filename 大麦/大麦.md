...
**问题场景**：更新数据库后，缓存中仍是旧数据，导致后续请求读到脏数据。

常见做法是先更新数据库，再操作缓存。而缓存操作又有两种主流方案：**延迟双删** 和 **更新后直接删除缓存**。
...
| 优点 | 说明 |
|------|------|
| 最终一致性强 | 第二次删除兜底，可消除并发读请求在①~②间写入缓存的旧数据 |
| 应对读写并发 | 有效解决「读线程写回旧缓存」的经典竞态问题 |

**缺点**：

| 缺点 | 说明 |
|------|------|
| 延迟时间难确定 | 延迟多久取决于读业务耗时，设置过长影响体验，过短兜底失效 |
| 第二次删除可能失败 | 需引入重试机制（如删除失败记录到消息表异步重试），增加复杂度 |
| 吞吐量下降 | 多一次缓存删除操作，写链路耗时增加 |
...
| 优点 | 说明 |
|------|------|
| 实现简单 | 只需一次删除，无延迟等待，无重试兜底逻辑 |
| 吞吐量高 | 写链路耗时最短，适合高并发写入场景 |
| 最终一致性达标 | 配合缓存的 TTL 过期机制，即使删除失败也能在 TTL 内自动收敛 |

**缺点**：

| 缺点 | 说明 |
|------|------|
| 短暂不一致窗口 | 更新数据库后、删除缓存前，并发读请求可能读到旧缓存（概率低，因为数据库更新通常在事务内，缓存删除紧随其后） |
| 删除失败场景 | 缓存删除失败会导致数据不一致，需要利用缓存 TTL 兜底 |
...
| 考量 | 说明 |
|------|------|
| 业务特性 | 大麦抢票场景下，库存和座位数据已经在 Redis 中通过 Lua 原子扣减，数据库是异步落库，写数据库后立即删缓存即可 |
| 一致性要求 | 最终一致性即可满足业务要求 |
| 性能优先 | 抢票场景写链路性能至关重要，减少一次额外开销 |
| TTL 兜底 | 即使删除失败，缓存 TTL 过期后自动拉取最新数据，保证最终一致 |
